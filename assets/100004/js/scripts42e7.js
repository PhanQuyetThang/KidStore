...
function getCartAjax() {
    ...
    jQuery('.cart_header_top_box').php('');
    jQuery.getJSON('/cart.js', function(cart, txtStatus) {
        if (cart.item_count != 0) {
            /*$('#CartCount').php(cart.item_count);
            $('#cartform').show();
            ...
            jQuery.each(cart.items, function(i, item) {
                ...
                if (item.image != null)
                    div.find('.cart_item_image').php("<img src=" + Haravan.resizeImage(item.image, 'small') + " alt=" + item.product_title + ">");
                else
                    div.find('.cart_item_image').php("<img src='//hstatic.net/0/0/global/noDefaultImage6_large.gif'>");
                vt = item.variant_options;
                if (vt.indexOf('Default Title') != -1)
                    vt = '';
                div.find('.cart_item_info .cart_item_title a').php(item.product_title + '<br><span>' + vt + '</span>').attr('href', item.url).attr('title', item.title);
                div.find('.cart_item_quantity').php("<input type='number' disabled min='1' id='update_" + item.variant_id + "' value='" + item.quantity + "' class='quantity_top_cart'/>");
                if (typeof(formatMoney) != 'undefined') {
                    div.find('.cart_item_price').php(Haravan.formatMoney(item.price, "") + "₫");
                    div.find('.total_cart .total_price').php(Haravan.formatMoney(total_price, "") + "₫");
                } else {
                    div.find('.cart_item_price').php(Haravan.formatMoney(item.price, "") + "₫");
                    div.find('.total_cart .total_price').php(Haravan.formatMoney(total_price, "") + "₫");
                }
                div.find('.remove').php("<a href='javascript:void(0);' onclick='deletecart(" + item.variant_id + ")' ><i class='fa fa-times'></i></a>");
            });
            /*$('.modal-title b').php(cart.item_count);
            $('.cart_header').find('.cart_header_count span').php('('+cart.item_count+')');
            $('.cart_header').find('.cart_price').php(Haravan.formatMoney(cart.total_price,'')+ "₫");
            $('.item-total').php(Haravan.formatMoney(cart.total_price,'')+ "₫");
            if(cart.item_count == 0){
            	$('#cart_popup #cartform').addClass('hidden');
            	$('#cart_popup #exampleModalLabel').php('Không có sản phẩm nào trong giỏ hàng của bạn.<p class="text-center">Tiếp tục mua hàng <a href="/collections/all">tại đây</a>.</p>');
            }
            else{
            	$('#cart_popup #exampleModalLabel').php('<img src="//hstatic.net/244/1000030244/10/2016/5-31/icon-cart.png" class="icon-cart"/> Giỏ hàng của bạn (' + cart.item_count + ' sản phẩm) <i class="fa fa-play" aria-hidden="true"></i>');
            ...
            jQuery('.cart_header').find('.box_text .cart_header_count span').php('(' + cart.item_count + ')');
            jQuery('.cart_header').find('.box_text .cart_price').php(Haravan.formatMoney(cart.total_price, '') + "₫");
        } else {
            /*$('#cart_popup #exampleModalLabel').php('Giỏ hàng của bạn đang trống. Mời bạn tiếp tục mua hàng.');
            $('#cartform').hide();*/
            jQuery('.cart_header').find('.box_text .cart_header_count span').php('(' + cart.item_count + ')');
            jQuery('.cart_header').find('.box_text .cart_price').php(Haravan.formatMoney(cart.total_price, '') + "₫");
            jQuery('.cart_header_top_box').php('<div class="cart_empty">Giỏ hàng của bạn vẫn chưa có sản phẩm nào.</div>');
        }
    });
}
...
function plus_quantity(btn, variant_id) {
    ...
    $(btn).parents('.line-item').find('.item-price').php(Haravan.formatMoney(new_price, '') + '₫');
    jQuery.ajax({
        type: 'POST',
        url: '/cart/change.js',
        dataType: 'json',
